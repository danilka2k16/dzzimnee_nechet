#include <iostream>
#include <stdio.h>//sprintf для перевода числа в чар [] массив символов
#include <string>//чтобы соединять строки плюсом
using namespace std;

struct Shot {//хранит имена броска и кол-во балоов за соотв бросок
    string name;
    int scores;
};
Shot* shots;
...
void initShot(){
    shots = new Shot[63]; //все варианты бросков
    string T="T",D="D",bull="Bull";
    char zz[3]; //для преобразования числа в const char* и добавления в строку
    int i=0;
...
void darts_game(int n) {
    ...
    for (int i = 0; i <= 62 && shots[i].scores <= n; i++)//еребор всех комб по 3 слаг (с 0)
